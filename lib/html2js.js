...
var createHtml2JsPreprocessor = function(logger, basePath, config) {
  ...
  return function(content, file, done) {
    ...
    //console.error("SPAWNING", origPath)
    var child = spawn("slimrb", [String(origPath)])
    //var child = spawn("slimrb", ["app/assets/javascripts/topics/topic/topic.html.slim"])

    child.stdout.on('data', function (buf) {
      var content = String(buf)
      //console.log(htmlPath, content)
      if (moduleName) {
